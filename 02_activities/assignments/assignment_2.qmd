...
#Run basic QC (MAF, missingness, and HWE filters)
```{bash}
...
#Generate a dataset of unrelated, QC'd individuals
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc.A2_pruned --king-cutoff 0.0884 --out ./02_activities/data/gwa.qc.A2_pruned
```
...
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc.A2 --keep ./02_activities/data/gwa.qc.A2_pruned.king.cutoff.in.id --make-bed --out ./02_activities/data/gwa.qc_unrelated.A2
```
...
#Select set of independent SNPs
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --extract ./02_activities/data/gwa.qc.A2.prune.in --make-bed --out ./02_activities/data/gwa.qc_unrelated.A2_pruned
```

#Run PCA
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2_pruned --pca 20 --out ./02_activities/data/gwa.qc.A2_unrel_PCA
```
...
#Run linear regression with the first 3 principal components as covariates
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --ci 0.95 --linear --covar ./02_activities/data/gwa.qc.A2_unrel_PCA.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.qc.A2_assoc
```
...
assoc_adjusted <- data.table::fread("./02_activities/data/gwa.qc.A2_assoc.PHENO1.glm.linear.adjusted")

assoc_add <- assoc[TEST == "ADD"]

colnames(assoc_add) = c("CHROM", "POS", "ID", "REF", "ALT", "PROVISIONAL_REF?", "A1", "OMITTED", "A1_FREQ", "TEST", "OBS_CT", "BETA", "SE", "L95", "U95", "T_STAT", "P", "ERRCODE")

# Manhattan plot
sig_snps = assoc_add$ID[assoc_add$P < 5e-8]
manhattan(assoc_add,
          chr = "CHROM",
          bp  = "POS",
          snp = "ID",
...
#Generate QQ plot of the GWAS p-values
```{r}
